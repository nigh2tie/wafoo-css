...
(function () {
  ...
  // --- utils ---
  function on(el, evt, handler, opts) { el && el.addEventListener(evt, handler, opts); }
    ...
    WFUI.tooltip = function (trigger, tip, opts) {
      ...
      function position() {
        ...
        if (chosen === 'left' || chosen === 'right') {
          ...
          var minL = 4, maxL = window.innerWidth - w - 4;
          left = clamp(left, minL, maxL);
          var minT = 4 + h/2, maxT = window.innerHeight - 4 - h/2;
      ...
      function show(v) { if (v) position(); tip.hidden = !v; tip.classList.toggle('is-open', v); }
      on(trigger, 'mouseenter', function () { show(true); });
      on(trigger, 'mouseleave', function () { show(false); });
      on(trigger, 'focus', function () { show(true); });
      on(trigger, 'blur', function () { show(false); });
      on(document, 'keydown', function (e) { if (e.key === 'Escape' && !tip.hidden) show(false); });
      on(window, 'scroll', function () { if (!tip.hidden) position(); }, { passive: true });
      on(window, 'resize', function () { if (!tip.hidden) position(); });
    };
  })();
  ...
  (function patchPopover() {
    ...
    WFUI.popover = function (trigger, panel, opts) {
      ...
      function shouldPrevent(trigger, e) {
        ...
        if (tag === 'INPUT') { var t = (trigger.type || '').toLowerCase(); return t === 'submit' || t === 'button'; }
        if (tag === 'A') { var href = trigger.getAttribute('href'); return !href || href === '#'; }
      ...
      var lastFocus = null;
      function open(v) {
        panel.hidden = !v; panel.classList.toggle('is-open', v);
        if (v) { lastFocus = document.activeElement; position(); setTimeout(function(){
          var f = panel.querySelector('a,button,input,select,textarea,[tabindex]:not([tabindex="-1"])');
          if (f) f.focus(); else panel.setAttribute('tabindex','-1'), panel.focus();
        },0);} else { if (lastFocus && lastFocus.focus) setTimeout(function(){ lastFocus.focus(); },0); }
      }
      ...
      on(document, 'click', function (e) { if (!panel.contains(e.target) && e.target !== trigger) open(false); });
      on(window, 'resize', function () { if (!panel.hidden) position(); });
      on(window, 'scroll', function () { if (!panel.hidden) position(); }, { passive: true });
      on(trigger, 'keydown', function (e) {
        if (e.key === 'Escape') open(false);
        if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); open(panel.hidden); }
      });
      on(panel, 'keydown', function (e) { if (e.key === 'Escape') open(false); });
    };
  })();
  ...
  (function patchSchedule() {
    ...
    function normalizeRange(range) {
      ...
      if (range === 'work1' || range === 'work2') {
        throw new Error('WFUI.schedule: timeRange "' + range + '" is not supported. Use API presets ("daytime"/"extended") or a custom object { start, end, label }');
      }
      ...
      if (typeof range === 'object') {
        var s = Number(range.start), e = Number(range.end);
        if (!(isFinite(s) && isFinite(e) && s >= 0 && e <= 24 && s < e)) {
          throw new Error('WFUI.schedule: invalid custom timeRange. Expect { start:number(0-23), end:number(1-24), label?:string } with start < end');
        }
    ...
    function applyCustomRange(root, customRange) {
      ...
      if (daily) {
        daily.querySelectorAll('.wf-schedule__time-slot').forEach(function (el) {
          ...
          var h = Number(t[0]||0), m = Number(t[1]||0);
          if (!customRange) {
    ...
    WFUI.schedule = function (root, opts) {
      ...
      if (dateInput) on(dateInput, 'change', function(){ hasDate = !!dateInput.value; applyGate(); if (inst.setSelectedDate) inst.setSelectedDate(dateInput.value || null); setTimeout(function(){ applyCustomRange(root, customRange); },0); });
      if (inst.setSelectedDate) { var origSet = inst.setSelectedDate; inst.setSelectedDate = function (d) { hasDate = !!d; applyGate(); var r = origSet.call(inst, d); setTimeout(function(){ applyCustomRange(root, customRange); },0); return r; }; }
      if (inst.setMode) { var origMode = inst.setMode; inst.setMode = function (m) { var r = origMode.call(inst, m); setTimeout(function(){ applyCustomRange(root, customRange); },0); return r; }; }
      if (inst.navigateWeek) { var origNav = inst.navigateWeek; inst.navigateWeek = function (dir) { var r = origNav.call(inst, dir); setTimeout(function(){ applyCustomRange(root, customRange); },0); return r; }; }
      if (inst.setTimeInterval) { var origTI = inst.setTimeInterval; inst.setTimeInterval = function (iv) { lastInterval = Number(iv||lastInterval); var r = origTI.call(inst, iv); setTimeout(function(){ applyCustomRange(root, customRange); },0); return r; }; }
      ...
      var mo = new MutationObserver(function(){
        ...
        if (help && help.innerHTML) {
          ...
          if (customRange) {
            ...
            var paren = '（' + label + ': ' + String(customRange.start).padStart(2,'0') + ':00-' + String(customRange.end).padStart(2,'0') + ':00、' + lastInterval + '分刻み）';
      ...
      setTimeout(function(){ applyCustomRange(root, customRange); }, 0);
      return inst;
    };
  })();
})();
